...
def save_eval_images(visuals, dir, id, opt):
    ...
    """
    img_i = visuals['input_I']
    img_s_pred = visuals['pred_Seg']
    img_s_real = visuals['real_Seg']

    save_image(img_i[:, :, ::-1], os.path.join(dir, str(id) + '_input.png'))
    save_image(img_s_real[:, :], os.path.join(dir, str(id) + '_seg_real.png'))
    save_image(img_s_pred[:, :], os.path.join(dir, str(id) + '_seg_pred.png'))
